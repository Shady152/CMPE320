...
void testJumble()
{
	...
	// Test the BadJumbleException Class ------------------------------------------------------------//
	cout << "Testing BadJumbleException:" << endl;
	// Tests that an error is thrown if the word has uppercase
	try
	{
		JumblePuzzle test0("UPPER", "medium");
	}
	catch (BadJumbleException &e)
	{
		cerr << e.what() << endl;
	...
	// Tests that an error is thrown if there are any non-alphabetic characters
	try
	{
		JumblePuzzle test1("cmpe320", "easy");
	}
	catch (BadJumbleException &e)
	{
		cerr << e.what() << endl;
	...
	// Test that an error is thrown if the input word is larger than 10 characters
	try
	{
		JumblePuzzle test2("parangaricutirimicuaro", "hard"); // fun fact this is actually a word
	}
	catch (BadJumbleException &e)
	{
		cerr << e.what() << endl;
	...
	// Test that an error is thrown if the input word is smaller than 3 characters
	try
	{
		JumblePuzzle test3("to", "hard");
	}
	catch (BadJumbleException &e)
	{
		cerr << e.what() << endl;
	...
	// Tests that an error is thrown if the difficult is invalid
	try
	{
		JumblePuzzle test5("upper", "potato");
	}
	catch (BadJumbleException &e)
	{
		cerr << e.what() << endl;
	...
	cout << endl;
	// End of BadJumbleException Class Test ---------------------------------------------------------//

	// Testing Accessors ----------------------------------------------------------------------------//
	cout << "Testing Accessors:" << endl;
	JumblePuzzle test5 = JumblePuzzle("sleep", "easy");
	showJumble(test5.getJumble(), test5.getSize());
	cout << "Expected Size: 10" << endl;
	cout << "Accessed Size: " << test5.getSize() << endl;

	cout << "Accessed Row and Column should matched with the word 'sleep' in the puzzle displayed above" << endl;
	cout << "Accessed Row: " << test5.getRowPos() << " | "
		 << "Accessed Column: " << test5.getColPos() << endl;

	cout << "Accessed Direction should match with the direction of the word 'sleep' displayed above" << endl;
	cout << "Accessed Direction: " << test5.getDirection() << endl;
	cout << endl;
	// End of Testing Accessors ---------------------------------------------------------------------//

	// Testing Size ---------------------------------------------------------------------------------//
	cout << "Testing Size and Form:" << endl;
	JumblePuzzle test6 = JumblePuzzle("lay", "easy");
	showJumble(test6.getJumble(), test6.getSize());
	cout << "Expected Size: 6 | Should match the size displayed above" << endl;
	cout << "Accessed Size: " << test6.getSize() << endl;

	JumblePuzzle test7 = JumblePuzzle("lay", "medium");
	showJumble(test7.getJumble(), test7.getSize());
	cout << "Expected Size: 9 | Should match the size displayed above" << endl;
	cout << "Accessed Size: " << test7.getSize() << endl;

	JumblePuzzle test8 = JumblePuzzle("lay", "hard");
	showJumble(test8.getJumble(), test8.getSize());
	cout << "Expected Size: 12 | Should match the size displayed above" << endl;
	cout << "Accessed Size: " << test8.getSize() << endl;
	cout << endl;
	cout << "For all of the above if they are square then they have the right form (the size of the rows and columns are the same)" << endl;
	cout << endl;
	// End of Testing Size --------------------------------------------------------------------------//

	// Testing the Null Constructor -----------------------------------------------------------------//
	cout << "Testing the NULL Constructor:" << endl;
	JumblePuzzle test10 = JumblePuzzle();
	showJumble(test10.getJumble(), test10.getSize());
	cout << endl;
	cout << "There should be no output above" << endl;
	cout << endl;
	// End of Null Constructor Testing --------------------------------------------------------------//

	/*
	NOTE TO THE TA: If you would like to make the words uppercase comment out line 25 and 26 in the constructor in the implementation file
	(jumble.cpp); since the instructions say that the input word must be lowercase then there is an exception in place to catch that so
	in order to play the game the and not have exceptions be thrown out but rather have the checks be output prior to running the game
	the words were changed to be in lowercase.
	*/
